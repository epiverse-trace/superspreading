# *superspreading*: Estimate individual-level variation in transmission

[superspreading](https://github.com/epiverse-trace/superspreading) is an
R package that provides a set of functions to estimate and understand
individual-level variation in transmission of infectious diseases from
data on secondary cases.
...
[superspreading](https://github.com/epiverse-trace/superspreading) from
[GitHub](https://github.com/) is to use the
...
it is used throughout the documentation of
[superspreading](https://github.com/epiverse-trace/superspreading) and
is a recommended package for those wanting to fit distribution models,
for example those supplied in
[superspreading](https://github.com/epiverse-trace/superspreading)
(Poisson-lognormal and Poisson-Weibull). We recommend reading the
...
Here we use
[`probability_epidemic()`](https://epiverse-trace.github.io/superspreading/dev/reference/probability_epidemic.md)
for the parameters estimated in the above section for Ebola, assuming
...
[superspreading](https://github.com/epiverse-trace/superspreading) can
be found in the [online documentation as package
...
[superspreading](https://github.com/epiverse-trace/superspreading) does
not provide plotting functions, instead we provide example code chunks
in the package’s vignettes that can be used as a templates upon which
data visualisations can be modified. We recommend users copy and edit
the examples for their own purposes. (This is permitted under the
...
Contributions to
[superspreading](https://github.com/epiverse-trace/superspreading) are
welcomed. Please follow the [package contributing
...
- [`{epichains}`](https://github.com/epiverse-trace/epichains) is
  ...
  [superspreading](https://github.com/epiverse-trace/superspreading) and
  [epichains](https://github.com/epiverse-trace/epichains) are: 1)
  [superspreading](https://github.com/epiverse-trace/superspreading) has
  functions to compute metrics that characterise outbreaks and
  superspreading events
  (e.g. [`probability_epidemic()`](https://epiverse-trace.github.io/superspreading/dev/reference/probability_epidemic.md),
  ...
  whereas [epichains](https://github.com/epiverse-trace/epichains) has
  functions to calculate the likelihood of a transmission chain size and
  length. 2) [epichains](https://github.com/epiverse-trace/epichains)
  exports functions to simulate a single-type branching process
  (`simulate_chains()` & `simulate_chain_stats()`).

- [`{modelSSE}`](https://CRAN.R-project.org/package=modelSSE) has a
  similar scope to
  [superspreading](https://github.com/epiverse-trace/superspreading), it
  contains functions to infer offspring distribution parameters. It
  exports several infectious disease outbreak datasets (see
  `data(package = "modelSSE")`). Both
  [superspreading](https://github.com/epiverse-trace/superspreading) and
  `{modelSSE}` export functions to calculate the proportion of
  transmission using different methods. It also imports the
  [`{Delaporte}`](https://CRAN.R-project.org/package=Delaporte) package
  to model the offspring distribution as a Delaporte distribution.
